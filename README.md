# Classification of Wine Types using the Gaussian Naive Bayes Model
...
## Dataset
 This project utilises the standard `load_wine` dataset to classify the target into **3 classes** - *class_0,class_1 and class_2* based upon it's features. This projects demonstrates the use of **Gaussian Naive Bayes** to *classifiy* the *dataset*.We create a *dataframe* using `panda library` to create the **features matrix ( X )** and **target vector ( y )**.
...
## Evaluation
 After applying the Gaussian Naive Bayes Model we got,
...
1.  **Clone the repository** (if you haven't already):
    ```bash
    git clone [https://github.com/Suchendra13/Wine_GaussianNB.git](https://github.com/Suchendra13/Wine_GaussianNB.git)
    cd Wine_GaussianNB
    ```
...
4.  **Open the Jupyter Notebook**:
    ```bash
    jupyter notebook Wine_GaussianNB.ipynb
    ```
